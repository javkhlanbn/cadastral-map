#!/usr/bin/env python3
"""
Скрипт для получения РЕАЛЬНЫХ координат всех кадастровых участков из Росреестра
"""
import pandas as pd
import json
import re
...
def get_coordinates_from_rosreestr(cadastral_number):
    """Получение РЕАЛЬНЫХ координат и границ участка из Росреестра"""
    try:
        ...
        # Вычисляем центр из координат границ
        coords = geometry.get("coordinates", [])
        if coords:
            # Для Polygon берем первый контур
            ...
            return {
                'lat': center_lat,
                'lng': center_lon,
                'geometry': geometry,  # Сохраняем полную геометрию границ
                'approximate': False
            }
        ...
        # print(f"Ошибка для {cadastral_number}: {e}")
        return None
...
def main():
    ...
    print(f"\n{'='*60}")
    print("Получаем РЕАЛЬНЫЕ координаты из Росреестра...")
    ...
    for i, lot in enumerate(lots):
        ...
        coordinates = get_coordinates_from_rosreestr(cadastral_number)
    ...
    print(f"\n{'='*60}")
    print(f"Результаты:")
    print(f"  Успешно получено: {success}/{total} ({success*100//total}%)")
